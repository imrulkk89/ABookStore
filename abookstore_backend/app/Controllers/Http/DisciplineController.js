...
class DisciplineController {
  async getDisciplineList({ response }) {
    const disciplines = await Discipline.all();

    return response.status(200).json({
      status: true,
      message: "All disciplines list populated",
      data: disciplines.toJSON()
    });
  }
  ...
  async getBooks({ params, response }) {
    ...
    try {
      ...
      const disciplineBooks = await Discipline.query()
        .with("books", builder => {
          builder
            .select([
              "id",
      ...
      return response.status(204).json({
        success: false,
        message: "No books found of this discipline."
      });
    }
  }
}
